...
# model validity conclusion: relatively satisfactory, we can use linear regression model.
```

Cross validation
purpose 1: study the stability of coefficients
purpose 2: prediction performance of the model
```{r}
...
final model with added observation
```{r}
final_model_new <- lm(data = X_new_scaled, death_rate ~ 
             num_social_security_ben_per_1000_resid +
             unemploy_rate + 
...
Model validation for model with added observation
```{r}
summary(final_model_new)
...
sort(mdl_cook_new, decreasing = TRUE)
```
...
2) Create another copy of the dataframe and introduce a special datapoint called 'Crazytown' (new dataset)
...
plot(y_hat_new, student_r_new, main = "X_new_scaled: Student residuals vs Y_hat") # 3rd assumption & 1st assumption & 2nd assumption
abline(h = 0, col = "red")
text(0,9,"Crazytown", col = "red")

plot(X_new_scaled[,13], student_r_new, main = "X_new_scaled: Student residuals vs\n num_social_security_ben_per_1000_resid") # good
abline(h = 0, col = "red")
text(0,9,"Crazytown", col = "red")
...
text(0,9,"Crazytown", col = "red")
```
Lastly, we conduct cross validation analysis for two purpose:
1) Study the stability of model coefficients
2) Study the stability of prediction performance of the model

We split our X_scaled dataset into 80% training and 20% validation sets. Then we run 1000 simulations and recorded each run's RMSE, MAE, and model coefficients.
```{r}
# model's original RMSE and MAE and coefficients
...
Next, we also plot the histogram for model coefficients.
```{r}
layout(matrix(c(1,1,2,2,3,3,4,4), nrow = 2, ncol = 4, byrow = TRUE))
...
```
We observe that from all four graphs, coefficient variability is minimal. 
