...
for (index_i in percent) {
  ...
  final_model_80 <- lm(data = X_scaled_80, death_rate ~ 
             ...
             per_cap_income_as_per_national_average)
  # calculate predict y
  predict_y_20 <- as.matrix(X_scaled_20[,c(6,11,12,13)]) %*%
    as.matrix(summary(final_model_80)[[4]][,1][-1], mode = "numeric") +
    rep(summary(final_model_80)[[4]][,1][1], length(X_scaled_20[,1]))
  # store the percentage error in the vector
  percent[index_i] <- mean(abs(predict_y_20-X_scaled_20[,1])/X_scaled_20[,1])
}
